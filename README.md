...
│   │   ├── Enums.cs                       # 列挙型定義
│   │   ├── ListItem.cs                    # コンボボックス等の汎用リストアイテム
│   │   ├── PrintMasterBase.cs             # 印刷設定基底
│   │   └── SubstrateInfo.cs               # 基板情報
│   │
│   ├── Data/                              # データアクセス層
│   │   ├── ProductRepository.cs           # 製品マスター参照
│   │   ├── SubstrateRepository.cs         # 基板マスター参照
│   │   ├── ProductRegistrationRepository.cs  # 製品登録
│   │   ├── SubstrateRegistrationRepository.cs # 基板登録
│   │   ├── SubstrateChangeRepository.cs   # 基板変更
│   │   ├── HistoryRepository.cs           # 履歴取得
│   │   ├── RePrintRepository.cs           # 再印刷履歴
│   │   ├── ServiceRepository.cs           # サービス向け製品参照
│   │   └── DbTransactionScope.cs          # トランザクション管理
│   │
│   ├── Common/                            # 共通ユーティリティ
│   │   ├── Constants.cs                   # DB テーブル名定数
│   │   ├── CommonUtils.cs                 # キーボード制御等の汎用処理
│   │   ├── BackupManager.cs               # DB バックアップ管理
│   │   ├── FileUtils.cs                   # ファイル操作ユーティリティ
│   │   └── Logger.cs                      # 操作ログ出力
│   │
│   ├── Services/                          # ドメインサービス
│   │   ├── BarcodeService.cs              # バーコード生成
│   │   ├── QrCodeParser.cs                # QR コード解析
│   │   ├── ExcelLauncher.cs               # Excel ファイル起動
│   │   ├── HistoryAuditLogger.cs          # 履歴監査ログ
│   │   └── SettingsLoader.cs              # 設定ファイル読み込み
│   │
│   ├── MasterManagement/                  # マスター管理機能
│   │   ├── MasterManagementWindow.cs      # 製品・基板マスター一覧・検索・ソート
│   │   ├── ProductMasterEditDialog.cs     # 製品マスター追加・編集ダイアログ
│   │   └── SubstrateMasterEditDialog.cs   # 基板マスター追加・編集ダイアログ
│   │
│   ├── Product/                           # 製品登録機能
│   │   ├── ProductRegistration1Window.cs  # Step1: 基本情報入力
│   │   ├── ProductRegistration2Window.cs  # Step2: 詳細確認・印刷
│   │   ├── ServiceForm.cs                 # サービス向け製品登録フォーム
│   │   ├── SubstrateChange1.cs            # 基板変更 Step1
│   │   └── SubstrateChange2.cs            # 基板変更 Step2
│   │
│   ├── Substrate/                         # 基板登録機能
│   │   └── SubstrateRegistrationWindow.cs
│   │
│   ├── History/                           # 履歴表示機能
│   │   ├── HistoryWindow.cs               # 履歴一覧・フィルタ表示
│   │   └── HistoryEditDialog.cs           # 履歴編集ダイアログ
│   │
│   ├── Print/                             # 印刷管理
│   │   ├── PrintManager.cs                # 印刷処理
│   │   ├── PrintOptions.cs                # 印刷オプション定義
│   │   └── PrintSettingsWindow.cs         # 印刷設定画面
│   │
│   ├── Excel/                             # 帳票生成
│   │   ├── ReportGeneratorClosedXml.cs    # 成績書生成
│   │   ├── CheckSheetGeneratorClosedXml.cs # チェックシート生成
│   │   ├── ListGeneratorClosedXml.cs      # 一覧表生成
│   │   ├── SubstrateInformationClosedXml.cs # 基板情報帳票生成
│   │   └── ExcelHelper.cs                 # Excel 共通処理
│   │
│   └── Other/                             # その他ダイアログ
│       ├── InputDialog1.cs                # 汎用入力ダイアログ
│       ├── SeveralDialogWindow.cs         # 複数選択ダイアログ
│       ├── LoadingOverlay.cs              # ローディング表示
